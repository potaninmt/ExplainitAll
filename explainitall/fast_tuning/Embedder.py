...
class GPTEmbedder():
  ...
  def get_embs_from_gpt(self, inp_str, n_layer_index = -1, head_index = 0, is_attention = False):
    ...
    if n_layer_index == 'all':
      return outputs.last_hidden_state[0,:,:].reshape((len(inp_tokens), self.model.config.n_embd)).to('cpu').detach().numpy()
    ...
    # Пройти заданное число gpt блоков
      ...
      if head_index == 'all':
        ...
        return_obj = outputs[1][n_layer_index][att_hidden][0, head_index, :, :].to('cpu').detach().numpy() # 0 - т.к. батч ожидается 1
